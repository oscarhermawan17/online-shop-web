...
const DATE_FILTERS: { label: string; value: DashboardPeriod }[] = [
...
export default function AdminDashboardPage() {
  ...
  if (productsLoading || ordersLoading || dashboardLoading) return <LoadingPage />;

  if (productsError || ordersError || dashboardError) {
    return (
      <ErrorMessage
  ...
  const getGrowthColor = (growth: number) => {
    if (growth > 0) return 'text-[#006f1d] bg-[rgba(145,247,142,0.2)]';
    if (growth < 0) return 'text-[#a73b21] bg-[rgba(253,121,90,0.2)]';
    return 'text-[#59615f] bg-[#e4e9e7]';
  };
        ...
        <div className="pt-20 px-4 pb-8 flex flex-col gap-6 bg-[#f1f4f2] min-h-screen">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center overflow-x-auto no-scrollbar bg-[#e4e9e7] rounded-xl p-1 gap-0">
                {DATE_FILTERS.map((filter) => (
                  <button
                    key={filter.value}
                    onClick={() => setPeriod(filter.value)}
                    className={`px-4 py-1.5 text-xs rounded-lg transition-colors whitespace-nowrap ${
                      period === filter.value
              ...
              {period === 'custom' && (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 text-xs bg-white border border-[#eaefec] rounded-lg text-[#2d3432] focus:outline-none focus:border-[#006f1d]"
                  />
                  <span className="text-[#59615f] text-xs">-</span>
                  <input
                    type="date"
                    value={endDate}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 bg-white rounded-2xl p-5 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] border border-[rgba(172,180,177,0.1)] relative overflow-hidden">
              ...
              <div className="flex items-baseline gap-1 mb-2">
                ...
                <span className="text-[#2d3432] font-extrabold text-3xl tracking-[-1.5px] leading-9">
                  {dashboardData?.totalSales.value.toLocaleString('id-ID')}
                </span>
              </div>
              <div className="flex items-center gap-1">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-sm ${getGrowthColor(dashboardData?.totalSales.growth ?? 0)}`}>
                  {formatGrowth(dashboardData?.totalSales.growth ?? 0)}
                </span>
              </div>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] border border-[rgba(172,180,177,0.1)] relative">
              ...
              <p className="text-[#59615f] text-[10px] font-semibold leading-[15px]">Total Pesanan</p>
              <div className="flex items-end justify-between">
                <p className="text-[#2d3432] text-xl font-bold leading-7">{dashboardData?.totalOrders.value}</p>
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-sm ${getGrowthColor(dashboardData?.totalOrders.growth ?? 0)}`}>
                  {formatGrowth(dashboardData?.totalOrders.growth ?? 0)}
                </span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] border border-[rgba(172,180,177,0.1)] relative">
              <div className="w-10 h-10 bg-[#f9fbb7] rounded-xl flex items-center justify-center mb-4">
                <Users className="w-5 h-5 text-[#5e602c]" />
              </div>
              <p className="text-[#59615f] text-[10px] font-semibold leading-[15px]">Pelanggan Baru</p>
              <div className="flex items-end justify-between">
                <p className="text-[#2d3432] text-xl font-bold leading-7">{dashboardData?.newCustomers.value}</p>
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-sm ${getGrowthColor(dashboardData?.newCustomers.growth ?? 0)}`}>
                  {formatGrowth(dashboardData?.newCustomers.growth ?? 0)}
                </span>
              </div>
            </div>
          </div>
          ...
          {/* Charts & Rankings (Mobile) */}
          <div className="flex flex-col gap-6">
            {/* Bar chart — Tren Penjualan */}
            <div className="bg-white rounded-2xl p-5 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] border border-[rgba(172,180,177,0.1)]">
              <h3 className="text-[#2d3432] text-sm font-bold leading-5 mb-4">Tren Penjualan</h3>
              <div className="h-56 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={dashboardData?.salesTrend || []} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#eaefec" />
                    <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: '#59615f', fontSize: 10 }} dy={10} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fill: '#59615f', fontSize: 10 }} tickFormatter={(val) => `Rp${(val / 1000000).toFixed(0)}M`} />
                    <Tooltip 
                      cursor={{ fill: '#f1f4f2' }}
                      contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0px 4px 6px -1px rgba(0,0,0,0.1)', fontSize: '12px' }}
                      formatter={(value) => [formatRupiah(typeof value === 'number' ? value : 0), 'Pendapatan']}
                    />
                    <Bar dataKey="sales" fill="#006f1d" radius={[4, 4, 0, 0]} maxBarSize={30} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            {/* Item Rankings */}
            <div className="bg-white rounded-2xl p-5 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] border border-[rgba(172,180,177,0.1)] flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[#2d3432] text-sm font-bold leading-5">Produk Terlaris</h3>
                <div className="flex items-center bg-[#e4e9e7] rounded-lg p-0.5">
                  <button
                    onClick={() => setRankingType('count')}
                    className={`px-2 py-1 text-[10px] font-semibold rounded-md transition-colors ${
                      rankingType === 'count' ? 'bg-white text-[#006f1d] shadow-sm' : 'text-[#59615f]'
                    }`}
                  >
                    Jumlah
                  </button>
                  <button
                    ...
                    className={`px-2 py-1 text-[10px] font-semibold rounded-md transition-colors ${
                      rankingType === 'value' ? 'bg-white text-[#006f1d] shadow-sm' : 'text-[#59615f]'
                    }`}
                  >
                    Nilai
                  </button>
              ...
              <div className="flex flex-col gap-3">
                {!rankings || rankings.length === 0 ? (
                  <p className="text-[#59615f] text-xs text-center py-4">Belum ada data</p>
                ) : (
                  rankings.slice(0, 5).map((item, index) => (
                    <div key={item.id} className="flex items-center gap-3">
                      <div className="w-6 h-6 rounded-full bg-[#f1f4f2] flex items-center justify-center shrink-0">
                        <span className="text-[#59615f] text-xs font-bold">{index + 1}</span>
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[#2d3432] text-xs font-semibold truncate">{item.name}</p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="text-[#006f1d] text-xs font-bold">
                          {rankingType === 'count' ? `${(item as { count?: number }).count ?? 0}x` : formatRupiah((item as { value?: number }).value ?? 0)}
                        </p>
                      </div>
                    </div>
                  ))
      ...
      <div className="hidden md:block space-y-8">
            ...
            {period === 'custom' && (
              <div className="flex items-center gap-2">
                <input
                  ...
                  value={startDate}
                  onChange={(e) => setStartDate(e.target.value)}
                  className="px-3 py-1.5 text-xs bg-white border border-[#eaefec] rounded-lg text-[#2d3432] focus:outline-none focus:border-[#006f1d]"
                />
                <span className="text-[#59615f] text-xs">-</span>
                <input
                  type="date"
        ...
        <div className="grid grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-xl p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] relative">
            <div className="flex items-center justify-between mb-14">
              ...
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${getGrowthColor(dashboardData?.totalOrders.growth ?? 0)}`}>
                {formatGrowth(dashboardData?.totalOrders.growth ?? 0)}
              </span>
            </div>
            <p className="text-[#59615f] text-sm font-medium leading-5">Total Pesanan</p>
            <p className="text-[#2d3432] text-3xl font-extrabold leading-9">{dashboardData?.totalOrders.value.toLocaleString('id-ID')}</p>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)]">
            <div className="flex items-center justify-between mb-14">
              ...
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${getGrowthColor(dashboardData?.newCustomers.growth ?? 0)}`}>
                {formatGrowth(dashboardData?.newCustomers.growth ?? 0)}
              </span>
            </div>
            <p className="text-[#59615f] text-sm font-medium leading-5">Pelanggan Baru</p>
            <p className="text-[#2d3432] text-3xl font-extrabold leading-9">{dashboardData?.newCustomers.value.toLocaleString('id-ID')}</p>
          </div>

          {/* Total Pendapatan — green gradient */}
          <div className="col-span-2 rounded-xl p-6 shadow-[0px_10px_15px_-3px_rgba(0,0,0,0.1)] relative overflow-hidden"
            ...
            <div className="flex items-center justify-between mb-14">
              ...
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full bg-[rgba(255,255,255,0.2)] text-white`}>
                {formatGrowth(dashboardData?.totalSales.growth ?? 0)}
              </span>
            </div>
            <p className="text-[rgba(234,255,226,0.8)] text-sm font-medium leading-5">Total Pendapatan (IDR)</p>
            <p className="text-[#eaffe2] text-3xl font-extrabold leading-9">{formatRupiah(dashboardData?.totalSales.value ?? 0)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          ...
          <div className="col-span-2 bg-white rounded-xl p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)]">
            <h3 className="text-[#2d3432] text-lg font-bold leading-7 mb-6">Tren Penjualan</h3>
            <div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dashboardData?.salesTrend || []} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#eaefec" />
                  <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: '#59615f', fontSize: 12 }} dy={10} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: '#59615f', fontSize: 12 }} tickFormatter={(val) => `Rp${(val / 1000000).toFixed(0)}M`} />
                  <Tooltip 
                    cursor={{ fill: '#f1f4f2' }}
                    contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0px 4px 6px -1px rgba(0,0,0,0.1)' }}
                    formatter={(value) => [formatRupiah(typeof value === 'number' ? value : 0), 'Pendapatan']}
                  />
                  <Bar dataKey="sales" fill="#006f1d" radius={[4, 4, 0, 0]} maxBarSize={40} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          {/* Item Rankings */}
          <div className="bg-white rounded-xl p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] flex flex-col">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-[#2d3432] text-lg font-bold leading-7">Produk Terlaris</h3>
              <div className="flex items-center bg-[#e4e9e7] rounded-lg p-0.5">
                <button
                  onClick={() => setRankingType('count')}
                  className={`px-2 py-1 text-[10px] font-semibold rounded-md transition-colors ${
            ...
            <div className="flex flex-col gap-4 flex-1 overflow-y-auto">
              {!rankings || rankings.length === 0 ? (
                <p className="text-[#59615f] text-sm text-center py-8">Belum ada data</p>
              ) : (
                rankings.map((item, index) => (
                  <div key={item.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[#2d3432] text-sm font-semibold truncate">{item.name}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-[#006f1d] text-sm font-bold">
                        {rankingType === 'count' ? `${(item as { count?: number }).count ?? 0}x` : formatRupiah((item as { value?: number }).value ?? 0)}
                      </p>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-3 gap-6">
          {/* Transaction table */}
          <div className="col-span-2 bg-white rounded-xl shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] overflow-hidden">
            ...
            {/* Table header */}
            <div className="grid grid-cols-[200px_1fr_120px_120px_60px] bg-[rgba(241,244,242,0.5)]">
            ...
            {last10Orders.length === 0 ? (
              ...
              last10Orders.map((order) => {
                ...
                return (
                  <div key={order.id} className="grid grid-cols-[200px_1fr_120px_120px_60px] border-t border-[#eaefec] items-center">
                    <div className="px-6 py-4 min-w-0">
                      <span className="text-[#2d3432] text-sm font-medium whitespace-nowrap truncate block">#{order.publicOrderId}</span>
                    </div>
                    <div className="flex items-center gap-3 px-6">
                      ...
                      <span className="text-[#2d3432] text-sm font-medium truncate">{order.customerName}</span>
                    </div>
                    ...
                    <div className="pr-6 py-4 flex justify-end">
          ...
          <div className="bg-white rounded-xl p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] flex flex-col gap-6">
            ...
            <Link
              href="/admin/products"
              className="mt-auto bg-[#e4e9e7] text-[#2d3432] text-sm font-semibold text-center py-2.5 rounded-lg hover:bg-[#d5e0db] transition-colors"
            >
