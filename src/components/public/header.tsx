...
export function Header({ storeName }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      try {
        ...
        const response = await api.get<{ data: ProductSearchSuggestion[] }>(
    ...
    const syncCurrentCustomer = async () => {
      try {
        const response = await api.get<{
          data: { customer: Parameters<typeof setAuth>[1] }
        }>("/customer-auth/me")
  ...
  const buildSearchHref = (rawQuery: string) => {
    const params = new URLSearchParams(
      pathname === "/catalog" ? searchParams.toString() : "",
    )
  ...
  const handleCekResi = () => {
    const trimmed = resiInput.trim()
    if (!trimmed) return
    setIsCekResiOpen(false)
    setResiInput("")
    ...
    <header className="sticky top-0 z-50 w-full backdrop-blur-md bg-white/90 border-b border-black/6 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-2 md:py-3">
        <div className="flex items-center justify-between gap-2 md:gap-4">
          ...
          <Link
            href="/"
            className="hidden md:block text-[#166534] font-bold text-xl leading-7 shrink-0"
          >
            {storeName}
          </Link>
