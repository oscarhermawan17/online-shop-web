...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-60 h-16 bg-white border-t border-black/10 shadow-[0px_-2px_10px_rgba(0,0,0,0.05)] md:hidden overflow-x-auto overflow-y-hidden [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
      <div className="flex h-full min-w-full w-max items-center scroll-px-1 px-1 snap-x snap-mandatory">
        {navItems.map(({ href, label, icon: Icon, exact }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex shrink-0 basis-1/5 min-w-[20%] snap-start flex-col items-center justify-center gap-1 py-1 transition-all duration-200 ${isActive ? 'scale-110' : 'scale-100'
                }`}
            >
