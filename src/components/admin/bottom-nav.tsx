...
const navItems = [
  ...
  { href: "/admin/notifications", label: "Notif", icon: Bell, exact: false },
  ...
  { href: "/admin/inventory", label: "Stok", icon: Boxes, exact: false },
          ...
          return (
            ...
            >
              <div className="relative">
                <Icon
                  className={`w-4.5 h-4.5 ${isActive ? "text-[#166534]" : "text-[#64748b]"}`}
                />
                {label === "Notif" && unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 h-2 w-2 rounded-full bg-[#dc2626]" />
                )}
              </div>
              <span
                className={`text-[10px] font-medium leading-3.75 ${isActive ? "text-[#166534]" : "text-[#64748b]"}`}
              >
                {label}
              </span>
            </Link>
